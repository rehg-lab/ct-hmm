function [cross_valid_set, subject_cv_map] = func_glaucoma_train_cross_validation(top_out_dir, eye_seq_list, learn_method, is_outer_soft, num_cv, cv_idx_list)
...
neighbor_link_setting = [1 1]; % forward link only
...
% this red->green color scheme is used in the NIPS paper
dwelling_time_draw_range_list = [0 2 5 100];
dwelling_time_draw_color_list = [255 0 0 ; 255 0 0; 0 255 0; 0 255 0];

%% this black-and-white color scheme is used by NYU now
%dwelling_time_draw_range_list = [0 10 100]; % this black-white drawing is by New York University's request
%%dwelling_time_draw_color_list = [255 255 255; 0 0 0; 0 0 0];
%dwelling_time_draw_color_list = [0 0 0; 255 255 255; 255 255 255];
...
if (num_cv > 1)
    ...
    subject_cv_map = ones(num_subject, 1);
end
...
%     CTHMM_MD_create_Q_mat_struct_supriya();
    
    %% init learning Q mat
    init_ave_state_dwell = 24.0; % 24 months
        
	if (learn_method == 3) %% this code added on 4.27.2018
		is_add_random_perturb = 1; % for eigen
		num_target_states = 3;
		esti_qrs = 1.0 / init_ave_state_dwell / num_target_states;
		
        perturb_amount = 0.05 * esti_qrs; % add noise on the initial qrs        
        %perturb_amount = 0.5;
