...
for e = 1:num_eye
    ...
    for k = 1:num_visit
        ...
        if (eye_list{e}.visit_list{k}.has_qual_OCT == 1 && visit.vf == 1 && isnan(visit.VFI) == 0 && isnan(visit.norm_rnfl_stratus) == 0)                
            data1 = visit.VFI;
...
output_dir = sprintf('%s/%s_%s', datasheet_folder, datasheet_name, str_Dx);
mkdir(output_dir);
...
%func_plot_datasheet_statistics(fp_log, output_dir, output_prefix, num_visit_list, trace_year_list, VFI_list, RNFL_list, baseline_age_list, age_list);
