function func_glaucoma_predict_separate_datasheet(test_eye_seq_list, pred_output_dir, pretrained_CTHMM_dir, pretrained_bayes_prior_model, pretrained_bayes_mean_sigma)

%% experiments for Glaucoma future measurements prediciton, use a pretrained HMM model
global num_min_hist_visit;
global obs_seq_list;
obs_seq_list = test_eye_seq_list;

%% create output dir
global out_dir;
out_dir = pred_output_dir;
str = sprintf('%s/log_pred.txt', out_dir);

global fp_log;
fp_log = fopen(str, 'wt');
pred_fig_dir = sprintf('%s/pred_fig_result', out_dir);
mkdir(pred_fig_dir);
...
bayes_prior_model = pretrained_bayes_prior_model;
bayes_mean_sigma = pretrained_bayes_mean_sigma;

%% load pretrained CT-HMM model
str = sprintf('%s/num_iter.txt', pretrained_CTHMM_dir);
fp = fopen(str, 'rt');
num_iter = fscanf(fp, '%d');
CTHMM_model_dir = sprintf('%s/Iter_%d', pretrained_CTHMM_dir, num_iter);    
...
for s = 1:num_seq  
    ...
    for v = 1:num_visit
        ...
        obs_seq_list{s}.ori_obs_seq(:, v) = obs_seq_list{s}.visit_list{v}.data'; 
    end
end
...
for s = 1:num_test_seq
    ...
    %% compute linear regression slope and bayesian linear regresison slope for the history visits
    ...
    %% prepare for recording prediction error
    num_visit = seq_data.num_visit;
    num_pred_visit = num_visit - num_min_hist_visit;    
...
%% visualize prediction results for each sequence
