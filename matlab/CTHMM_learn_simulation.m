function CTHMM_learn_simulation(out_directory, syn_qi_range, test_method_list, syn_data_config_list, run_list, num_dim, num_state_per_dim, neighbor_setting)
...
for r = 1:num_run
    ...
    %e.g. for a 2D forwarding model (like in glaucoma experiment), neighbor_link_setting = [1 1]; 
    % 1: forward link only, 2: backward link only, 3: both directions, 4:
    % fully connected model
    CTHMM_sim_gen_syn_Q_mat(syn_qi_range, neighbor_setting);
    
    disp("this is Q_mat_struct")
    global Q_mat_struct
    Q_mat_struct
...
%         if (is_compute_ground_truthQ_mat_Q_log_likelihood == 1) 
%             Q_mat = syn_Q_mat;
