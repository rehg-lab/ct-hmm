...
if (is_joint_inference == 1)
    ...
    %% print out mean
    num_para = length(gm_list{1}.mu);
    
    fprintf(fp_log, 'Fitted Gaussian: mean vector:\n');
    for d = 1:num_para
        fprintf(fp_log, '%f ', gm_list{1}.mu(d));
    end
    fprintf(fp_log, '\n');
    
    %% print out covariante matrix
    fprintf(fp_log, 'Fitted Gaussian: covariance matrix:\n');
    for r = 1:num_para
        for c = 1:num_para
            fprintf(fp_log, '%f ', gm_list{1}.Sigma(r,c));
        end
        fprintf(fp_log, '\n');
    end
    
    fprintf(fp_log, 'Fitted Gaussian: standard deviation:\n');
    for r = 1:num_para
        std_para = sqrt(gm_list{1}.Sigma(r,r));
        fprintf(fp_log, '%f ', std_para);        
    end
    fprintf(fp_log, '\n');
