...
for d = 1:num_dim   
    ...
    str = sprintf('(Global LR abs error: %.4f +- %.4f. This uses all data to find the LR line. Not a testing method.)\n', ave_global_LR_abs_err, std_global_LR_abs_err);
    CTHMM_print_log(str);
    ...
    if (num_min_hist_visit >= 2)    
        ...
        [h,p,ci,stats] = ttest(overall_bayes_LR_abs_err(d, :) - overall_LR_abs_err(d, :))
        str = sprintf('bayes LR vs LR: h = %f, p = %f, ci = %f, %f\n\n', h, p, ci(1), ci(2));
        CTHMM_print_log(str);
    end
