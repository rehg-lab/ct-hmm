...
elseif (syn_config_idx >= 20 && syn_config_idx <= 23) % config 20-24 is for decoding simulation, state noise level 0.25 (sigma of gaussian)
    state_sigma = 0.25;
    total_visit_list(1) = [10000];    
    hold_time_para_ls = [0.5 1 2 4];    
    time_intv_list{1} = smallest_hold_time * hold_time_para_ls(syn_config_idx-19);
    dur_list(1) = largest_hold_time * 100;  
    
elseif (syn_config_idx >= 24 && syn_config_idx <= 27) % config 24-27 is for decoding simulation, state noise level 0.50 (sigma of gaussian)
    state_sigma = 0.50;
    total_visit_list(1) = [10000];    
    hold_time_para_ls = [0.5 1 2 4];    
    time_intv_list{1} = smallest_hold_time * hold_time_para_ls(syn_config_idx-23);
    dur_list(1) = largest_hold_time * 100;            
end
%==================================================
