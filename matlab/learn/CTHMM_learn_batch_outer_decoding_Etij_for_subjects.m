...
%         [Evij, subject_log_prob, Pt_list] = CTHMM_decode_outer_forwbackw_supriya(obs_seq_list{subject_idx});
    else
    ...
    if (subject_log_prob == inf)
        ...
        disp('subject_log_prob = inf, exit'); 
        CTHMM_print_log(str);
        exit;
    end
    
    if (isnan(subject_log_prob) == 1)
        str = sprintf('subject_log_prob = NaN for subject = %d\n', g);                
        disp('subject_log_prob = NaN, exit'); 
        CTHMM_print_log(str);
        exit;
    end
