...
for s = 1:num_state
    ...
    for t = 1:num_state
        ...
        for d = 1:dim  % check each dimension  
            ...
            if (neighbor_link_setting(1) == 4)
            
            elseif (neighbor_link_setting(1) == 1) % forward link only
            ...
            elseif (neighbor_link_setting(1) == 2) % backward link only
                if ((t_dim_states(d) - s_dim_states(d)) == 0 || (t_dim_states(d) - s_dim_states(d)) == -1)                               
                else
                    is_neighbor = 0;
                    break;
                end                
            elseif (neighbor_link_setting(1) == 3) % both forward and backward link are allowed
